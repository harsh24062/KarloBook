...
const NavBar = () => {
  ...
  const {user} = useUser()
  const {openSignIn} = useClerk()  // openSignIn is in-build

  const navigate = useNavigate()

  return (
    <div className="fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-14 lg:px-20 py-0">
        {/* Logo */}
        <Link to='/' className="max-md:flex-1">
          <img src={assets.logo} alt="Logo_Image" className="w-28 h-28"/>
        </Link>
        {/* Menu Items */}
        <div className={`max-md:absolute max-md:top-0 max-md:left-0 max-md:font-medium
          ...
          bg-black/70 md:bg-white/20 md:border border-gray-300/20 overflow-hidden
        ...
        <div className="flex items-center gap-8">
           ...
           {!user?(
             <button className="px-4 py-1 sm:px-7 sm:py-2 bg-primary hover:bg-primary-dull 
                ...
                // User Profile will display 
                <UserButton>
                  {/* Adding my-bookings button option in clerk menu */}
